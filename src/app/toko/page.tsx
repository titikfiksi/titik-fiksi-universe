    ...
    <div className="min-h-screen bg-gray-50 pt-32 pb-20">
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        <div className="space-y-16">
          ...
          {donations.length > 0 && (
            <section>
              <div className="flex items-center gap-3 mb-6 border-b border-gray-200 pb-4">
                ...
                <h2 className="text-2xl md:text-3xl font-black text-gray-900">Saluran Donasi</h2>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {donations.map((donasi) => (
                  <a key={donasi.id} href={donasi.url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between bg-white p-5 rounded-2xl border border-pink-100 shadow-sm hover:shadow-md hover:border-pink-300 transition group">
                    <span className="font-bold text-gray-800 uppercase group-hover:text-pink-600">{donasi.platform}</span>
          ...
          {/* BAGIAN PRODUK (DENGAN SLIDER) */}
          <section>
            <div className="flex items-center gap-3 mb-6 border-b border-gray-200 pb-4">
              <ShoppingBag className="text-indigo-600" size={28} />
              <h2 className="text-2xl md:text-3xl font-black text-gray-900">Produk Rekomendasi</h2>
            </div>

            {sponsors.length === 0 ? (
              <div className="text-center py-16 bg-white border-2 border-dashed border-gray-200 rounded-3xl">
                <ShoppingBag size={48} className="mx-auto text-gray-300 mb-4" />
                <p className="text-gray-500 font-bold">Belum ada produk yang ditampilkan saat ini.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 md:gap-8">
                {sponsors.map((sponsor) => (
                  <div key={sponsor.id} className="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden flex flex-col hover:shadow-xl hover:border-indigo-200 transition-all duration-300">

                    {/* PEMANGGILAN KOMPONEN SLIDER CERDAS */}
                    <ProductSlider images={sponsor.imageUrl.split(',')} title={sponsor.title} />
                    ...
                    <div className="p-5 md:p-6 flex flex-col flex-grow">
                      ...
                      <a href={sponsor.linkUrl} target="_blank" rel="noopener noreferrer" className="mt-auto w-full bg-indigo-600 text-white font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 hover:bg-indigo-700 transition shadow-md hover:shadow-lg transform hover:-translate-y-0.5">

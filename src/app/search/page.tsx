...
export default async function SearchPage({ searchParams }: { searchParams: { q?: string, genre?: string, p?: string } }) {
  const query = searchParams.q || "";
  const genre = searchParams.genre || "";
  
  // LOGIKA PAGINASI
  const currentPage = parseInt(searchParams.p || "1") || 1;
  ...
  // Menarik data settings, total novel, dan data novel per halaman
  ...
  // Fungsi untuk membuat URL paginasi yang tetap mempertahankan kata kunci pencarian
    ...
    <div className="min-h-screen pb-20 pt-32 overflow-x-hidden bg-gray-50">
      <div className="container mx-auto px-4 max-w-5xl">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <div>
            <Link href="/" className="inline-flex items-center gap-2 text-gray-500 hover:text-blue-600 font-bold mb-4 transition">
          ...
          <form className="relative w-full md:w-96" action="/search" method="GET">
            <input 
              type="text" 
              name="q" 
              defaultValue={query}
            ...
            <button type="submit" className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-gray-900 text-white px-4 py-1.5 rounded-full text-sm font-bold hover:bg-gray-800 transition">
              Cari
            </button>
          </form>
        </div>
        ...
        {query || genre ? (
           <div className="mb-8 p-4 bg-blue-50 text-blue-700 font-medium rounded-2xl flex items-center justify-between gap-2">
             <span>Menampilkan hasil untuk: <strong>{query || genre}</strong></span>
             <span className="text-sm font-bold bg-blue-100 px-3 py-1 rounded-full">{totalNovels} Ditemukan</span>
           </div>
        ) : null}
        ...
        {novels.length === 0 ? (
          <div className="text-center py-24 border-2 border-dashed border-gray-200 rounded-3xl bg-white">
            ...
            <p className="text-gray-500">Coba gunakan kata kunci atau kategori genre yang berbeda.</p>
          </div>
        ) : (
          <>
            {/* GRID KARTU NOVEL (TIDAK ADA PERUBAHAN DESAIN) */}
              ...
              {novels.map((novel) => {
                ...
                return (
                  <Link key={novel.id} href={`/novel/${novel.slug}`} className="group flex flex-col bg-white border border-gray-100 rounded-3xl p-2 hover:border-blue-200 hover:shadow-xl transition-all duration-300">
                    ...
                    <h3 className="font-bold text-gray-900 text-sm leading-tight group-hover:text-blue-600 transition-colors line-clamp-2 px-1">{novel.title}</h3>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 mt-12 pt-8 border-t border-gray-200">
                <Link 
                  ...
                  className={`p-2 rounded-xl border transition-all flex items-center justify-center ${currentPage > 1 ? 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:border-blue-300' : 'border-gray-100 bg-gray-50 text-gray-300 cursor-not-allowed'}`}
                >
                  <ChevronLeft size={20} />
                </Link>
                ...
                <div className="flex items-center gap-1 px-2">
                  {Array.from({ length: totalPages }).map((_, i) => {
                    const pageNum = i + 1;
                    // Hanya tampilkan beberapa halaman di sekitar halaman aktif agar tidak terlalu panjang
                    if (pageNum === 1 || pageNum === totalPages || (pageNum >= currentPage - 1 && pageNum <= currentPage + 1)) {
                      return (
                        <Link 
                          key={pageNum} 
                          href={createPageUrl(pageNum)}
                          className={`w-10 h-10 flex items-center justify-center rounded-xl text-sm font-bold transition-all ${pageNum === currentPage ? 'bg-blue-600 text-white shadow-md' : 'bg-white border border-gray-200 text-gray-600 hover:border-blue-300 hover:text-blue-600'}`}
                        >
                          {pageNum}
                        </Link>
                      );
                ...
                <Link 
                  ...
                  className={`p-2 rounded-xl border transition-all flex items-center justify-center ${currentPage < totalPages ? 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:border-blue-300' : 'border-gray-100 bg-gray-50 text-gray-300 cursor-not-allowed'}`}
                >

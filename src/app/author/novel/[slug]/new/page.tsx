...
// Helper for local ISO datetime for input default value
const getLocalIsoDatetime = (): string => {
  const now = new Date();
  const tzOffset = now.getTimezoneOffset() * 60000;
  return new Date(now.getTime() - tzOffset).toISOString().slice(0, 16);
};

// Slugify function for client-side dynamic slug generation
const slugifyScript = `
(function() {
...
interface AuthorNewChapterPageProps {
  params: { slug: string };
}

const AuthorNewChapterPage: FC<AuthorNewChapterPageProps> = async ({ params }) => {
  // Auth & Permissions
  ...
  try {
    verified = await jwtVerify(token, getJwtSecret());
  } catch {
  ...
  const localISOTime = getLocalIsoDatetime();
        ...
        <form action={createWithParams} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            <div className="md:col-span-2 space-y-2">
              <label className="text-xs font-black text-gray-400 uppercase tracking-widest" htmlFor="orderIndex">No. Bab</label>
              <input
                type="number"
                name="orderIndex"
                id="orderIndex"
                ...
                required
                className="w-full p-4 bg-indigo-50 border border-indigo-200 text-indigo-800 rounded-xl outline-none font-black text-center text-xl"
                min={1}
                readOnly
              />
            </div>
            <div className="md:col-span-6 space-y-2">
              <label className="text-xs font-black text-gray-400 uppercase tracking-widest" htmlFor="titleInput">Judul Bab</label>
              <input
                type="text"
                name="title"
                id="titleInput"
                ...
                className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl outline-none font-bold text-gray-800"
                autoComplete="off"
              />
            </div>
            <div className="md:col-span-4 space-y-2">
              <label className="text-xs font-black text-gray-400 uppercase tracking-widest" htmlFor="slugInput">Slug URL</label>
              <input
                type="text"
                name="slug"
                id="slugInput"
                required
                placeholder="pertemuan-di-hutan"
                className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl outline-none font-mono text-sm"
                pattern="^[a-z0-9]+(?:-[a-z0-9]+)*$"
                title="Hanya huruf kecil, angka, dan tanda minus."
                autoComplete="off"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-gray-100">
            <div className="bg-amber-50 p-5 rounded-2xl border border-amber-100 space-y-3">
              ...
              <input
                type="url"
                name="payLink"
                placeholder="Link Trakteer / Saweria khusus bab ini"
              ...
              <input
                type="text"
                name="unlockCode"
                placeholder="Kode Buka Kunci (Opsional)"
            ...
            <div className="space-y-4">
              <div className="bg-emerald-50 p-5 rounded-2xl border border-emerald-100 space-y-3">
                ...
                <p className="text-[10px] text-emerald-700 font-bold">
                  *Jika tanggal diatur ke masa depan, bab ini baru akan terlihat oleh pembaca pada tanggal tersebut.
                </p>
              </div>
              <SubmitButton
                text="Simpan & Unggah Bab"
                customClass="w-full flex items-center justify-center gap-2 bg-indigo-600 text-white font-bold py-3 rounded-xl hover:bg-indigo-700 transition shadow-sm text-sm"
                isEdit={false}
              />
            </div>
          </div>
        </form>
        {/* No inline JS: inject sanitized script for slugify input */}
        <script dangerouslySetInnerHTML={{ __html: slugifyScript }} />
      </div>
    </div>
  );
};

export default AuthorNewChapterPage;
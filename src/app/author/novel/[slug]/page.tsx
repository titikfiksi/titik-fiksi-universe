...
import {
  deleteChapterByAuthor,
  addExternalLinkByAuthor,
  deleteExternalLinkByAuthor,
  addAuthorSocialByAuthor,
  deleteAuthorSocialByAuthor,
  replyCommentByAuthor,
} from "@/lib/actions";
import DeleteButton from "@/components/DeleteButton";

// --- JWT Setup ---
const secretKey = process.env.JWT_SECRET;
if (!secretKey) throw new Error("JWT_SECRET tidak ditemukan di environment variables!");
...
export default async function AuthorManageNovelPage({ params }: Params) {
  // -- Auth, redirect if not logged in or invalid JWT
  ...
  // -- Collect latest 15 comments from all chapters, flattened & enriched
  const allComments = novel.chapters
    .flatMap((chapter) =>
    ...
    if (novel.chapters.length === 0) {
      return (
        <p className="text-center py-10 text-gray-500 font-medium bg-gray-50 rounded-2xl border border-dashed border-gray-200">
          Belum ada bab yang ditulis. Ayo mulai berkarya!
        </p>
      );
    }
    return (
      <div className="space-y-3">
        {novel.chapters.map((chapter) => (
          <div
            key={chapter.id}
            className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 group hover:border-indigo-200 transition"
          >
            <div className="flex items-center gap-4 overflow-hidden pr-4">
              <div className="w-10 h-10 bg-indigo-100 text-indigo-700 rounded-xl flex items-center justify-center font-black flex-shrink-0 text-sm">
                {chapter.orderIndex}
              </div>
              <div className="truncate">
                ...
                <div className="flex flex-wrap items-center gap-3 mt-1.5 text-[10px] font-bold text-gray-400">
                  {chapter.isPublished ? (
                    <span className="text-emerald-500">Telah Tayang</span>
                  ) : (
                    <span className="text-amber-500">Jadwal / Draf</span>
                  )}
                  {chapter.isLocked && (
                    <span className="flex items-center gap-1 text-amber-500">
                      <Lock size={10} /> Eksklusif (Premium)
                    </span>
                  )}
                </div>
              </div>
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <Link
                href={`/author/novel/${novel.slug}/edit/${chapter.id}`}
                className="p-2.5 bg-white border border-gray-200 text-amber-500 hover:bg-amber-50 rounded-xl transition shadow-sm"
                aria-label={`Edit Bab ${chapter.orderIndex}`}
              >
                <Edit size={14} />
              </Link>
              <form action={deleteChapterByAuthor.bind(null, chapter.id, novel.id, novel.slug)}>
                <DeleteButton message={`Hapus Bab ${chapter.orderIndex} permanen?`} />
              </form>
            </div>
          </div>
        ))}
        ...
        {allComments.map((comment) => (
          ...
          >
            ...
            <details className="mt-4 group/reply">
              ...
              <form
                action={replyCommentByAuthor.bind(null, comment.chapterId, novel.slug)}
              ...
              >
                ...
                <input
                  type="text"
                  name="content"
                  placeholder={`Sapa @${comment.name} sebagai Penulis...`}
                  required
        ...
        {novel.externalLinks.map((link) => (
          ...
          >
            <div className="overflow-hidden pr-2">
              <span className="font-bold text-blue-900 text-xs block truncate">{link.title}</span>
            </div>
            <form action={deleteExternalLinkByAuthor.bind(null, link.id, novel.id, novel.slug)}>
      ...
      <div className="space-y-2">
        {novel.authorSocials.map((social) => (
          ...
          >
            ...
            <form action={deleteAuthorSocialByAuthor.bind(null, social.id, novel.id, novel.slug)}>
              <DeleteButton message="Hapus?" />
            </form>
          </div>
        ))}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-200 pb-6">
        <div>
          <h1 className="text-3xl font-black text-gray-900 flex items-center gap-3">
            ...
            Ruang <span className="text-indigo-600">Karya</span>
          </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="bg-white p-6 md:p-8 rounded-3xl border border-gray-200 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4 border-b border-gray-100 pb-4">
              <h2 className="font-bold text-gray-800 flex items-center gap-2">
                <Layout size={18} /> Kelola Bab Cerita
              </h2>
              <Link
                href={`/author/novel/${novel.slug}/new`}
                className="inline-flex items-center gap-2 bg-indigo-600 text-white px-5 py-2 rounded-xl text-sm font-bold hover:bg-indigo-700 transition shadow-md"
              >
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm relative overflow-hidden">
            ...
            <h2 className="font-bold text-gray-800 mb-2 flex items-center gap-2 relative z-10">
              <Heart className="text-pink-500" size={18} />
              Donasi Utama Cerita
            </h2>
            <p className="text-xs text-gray-500 mb-4 relative z-10">
              Link Trakteer/Saweria ini akan muncul di luar bab premium agar pembaca bisa memberi tip sukarela.
            </p>
            <div className="p-4 bg-pink-50 border border-pink-100 rounded-xl relative z-10">
              {novel.authorDonationUrl ? (
                <a
                  href={novel.authorDonationUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-sm font-bold text-pink-700 break-all hover:underline"
                >
                  {novel.authorDonationUrl}
                </a>
              ) : (
                <p className="text-xs font-bold text-pink-400">
                  Belum ada link donasi diatur.
                </p>
              )}
            </div>
            <p className="text-[10px] font-bold text-gray-400 mt-3 text-center">
              *Ubah link donasi utama melalui menu Edit Novel
            </p>
          </div>

          {/* External Links (Promosi Platform Lain) */}
          <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm">
            <h2 className="font-bold text-gray-800 mb-4 flex items-center gap-2 pb-4 border-b border-gray-100">
              <LinkIcon className="text-blue-600" size={18} />
              Promosi Platform Lain
            </h2>
            <form
              action={addExternalLinkByAuthor.bind(null, novel.id, novel.slug)}
              className="space-y-3 mb-6"
            >
              <input
                type="text"
                name="title"
                placeholder="Contoh: Baca juga di KBM"
                required
                className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-blue-600"
              />
              <input
                type="url"
                name="url"
                placeholder="https://..."
                required
                className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-600"
              />
              <button
                type="submit"
                className="w-full flex items-center justify-center gap-2 bg-blue-600 text-white py-3 rounded-xl text-sm font-bold hover:bg-blue-700 transition shadow-sm"
              >
          ...
          <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm">
            <h2 className="font-bold text-gray-800 mb-4 flex items-center gap-2 pb-4 border-b border-gray-100">
              <AtSign className="text-emerald-600" size={18} />
              Hubungkan Sosial Media
            </h2>
            <form
              action={addAuthorSocialByAuthor.bind(null, novel.id, novel.slug)}
              className="space-y-3 mb-6"
            >
              <input
                type="text"

...
export default async function AuthorDashboardPage() {
  ...
  const promoMessage = encodeURIComponent(`Halo Admin TF Universe, saya penulis atas nama ${user?.name}. Saya tertarik untuk menyewa Banner Sorotan Utama untuk mempromosikan karya saya.`);
  const waLink = cleanWaNumber ? `https://wa.me/${cleanWaNumber}?text=${promoMessage}` : "#";

  return (
    <div className="max-w-6xl mx-auto animate-fade-in-up space-y-8">
      
      {/* 1. SPANDUK SELAMAT DATANG */}
      <div className="bg-gradient-to-br from-indigo-700 via-blue-600 to-indigo-900 rounded-[2.5rem] p-8 md:p-10 text-white shadow-xl relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-6 border border-indigo-500/30">
        <div className="relative z-10 flex-1">
          ...
          <h1 className="text-3xl md:text-5xl font-black mb-3 drop-shadow-lg">Halo, {user?.name}!</h1>
      ...
      {/* 2. PUSAT INFO & PROMOSI (TERKONEKSI KE DATABASE ADMIN) */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        
        <div className="bg-white rounded-[2rem] border border-gray-100 p-6 md:p-8 shadow-sm flex flex-col">
          <div className="flex items-center gap-3 mb-6 border-b border-gray-50 pb-4">
            <div className="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center"><Megaphone size={20} /></div>
            <div>
          ...
          <div className="space-y-4 flex-1">
            <div className="group flex gap-4 items-start">
              <div className="w-2 h-2 rounded-full bg-blue-500 mt-2 relative"><div className="absolute inset-0 bg-blue-500 animate-ping rounded-full opacity-50"></div></div>
              <div>
                <h4 className="text-sm font-bold text-gray-900 group-hover:text-blue-600 transition">{settings?.authorAnnounce1Title || "Selamat Datang di Dasbor Baru!"}</h4>
                <p className="text-xs text-gray-500 mt-1 leading-relaxed">{settings?.authorAnnounce1Desc || "Sistem Titik Fiksi Universe v2.0 telah aktif. Pastikan Anda membaca Panduan Penulis untuk memahami fitur-fitur baru seperti Kunci Bab Premium."}</p>
              </div>
            </div>
            <div className="group flex gap-4 items-start pt-4 border-t border-gray-50">
              <div className="w-2 h-2 rounded-full bg-gray-300 mt-2"></div>
              <div>
                <h4 className="text-sm font-bold text-gray-900 group-hover:text-blue-600 transition">{settings?.authorAnnounce2Title || "Kebijakan Kreator Penulis"}</h4>
                <p className="text-xs text-gray-500 mt-1 leading-relaxed">{settings?.authorAnnounce2Desc || "Harap pastikan semua karya yang dipublikasikan mematuhi pedoman komunitas kami dan tidak mengandung unsur plagiarisme."}</p>
                <Link href="/author/guide" className="text-[10px] font-black text-blue-600 uppercase tracking-widest mt-2 flex items-center gap-1 hover:underline">Baca Panduan <ChevronRight size={12}/></Link>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-[2rem] border border-gray-700 p-6 md:p-8 shadow-2xl relative overflow-hidden flex flex-col justify-between group">
          ...
          <div className="relative z-10">
            ...
            <p className="text-gray-400 text-sm leading-relaxed mb-6">
              {settings?.promoPremiumDesc || "Dapatkan ribuan pembaca baru dengan menampilkan novelmu di Banner Raksasa pada halaman utama Titik Fiksi Universe. Slot sangat terbatas!"}
            </p>
          </div>
          <a href={waLink} target="_blank" rel="noopener noreferrer" className="relative z-10 bg-gradient-to-r from-amber-500 to-orange-500 text-white px-6 py-3.5 rounded-xl text-sm font-black flex items-center justify-between hover:from-amber-400 hover:to-orange-400 transition-all shadow-lg shadow-amber-500/20 active:scale-95 group/btn">
            Pesan Slot Promosi Sekarang <ArrowRight size={18} className="group-hover/btn:translate-x-1 transition-transform"/>
          </a>
        </div>
      ...
      <div className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 border-b border-gray-100 pb-4">
          <h2 className="text-2xl font-black text-gray-900 flex items-center gap-2"><BookOpen className="text-indigo-600" /> Pustaka Karyamu</h2>
          <Link href="/author/new" className="bg-indigo-600 text-white px-6 py-3 rounded-xl text-sm font-black flex items-center justify-center gap-2 hover:bg-indigo-700 transition shadow-lg hover:shadow-indigo-500/30 active:scale-95"><Plus size={18}/> Buat Novel Baru</Link>
        </div>

        {userNovels.length === 0 ? (
           <p className="text-center py-10 text-gray-500 font-medium bg-gray-50 rounded-2xl border border-dashed border-gray-200">Belum Ada Karya</p>
        ) : (

...
export default async function AuthorDashboardPage() {
  ...
  // Ambil data user, novel, dan settings secara paralel
  ...
  // Render judul promosi dengan penekanan kata
  function renderPromoPremiumTitle(title?: string) {
    if (!title) return <>Jadikan Karyamu <span className="text-amber-400">Sorotan Utama!</span></>;
    const words = title.split(" ");
    if (words.length < 2) return title;
    const before = words.slice(0, -2).join(" ");
    const after = words.slice(-2).join(" ");
    return <>{before} <span className="text-amber-400">{after}</span></>;
  }
    ...
    <div className="max-w-6xl mx-auto animate-fade-in-up space-y-8">
      ...
      <section className="bg-gradient-to-br from-indigo-700 via-blue-600 to-indigo-900 rounded-[2.5rem] p-8 md:p-10 text-white shadow-xl relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-6 border border-indigo-500/30">
        <div className="relative z-10 flex-1">
          ...
          <p className="text-indigo-100 font-medium max-w-lg leading-relaxed opacity-90 text-sm">
            Selamat datang di pusat komando Anda. Mulailah menulis bab baru dan kembangkan sayap karyamu di Titik Fiksi Universe.
          </p>
        </div>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-[2rem] border border-gray-100 p-6 md:p-8 shadow-sm flex flex-col">
          ...
          <div className="space-y-4 flex-1">
            <div className="group flex gap-4 items-start">
              ...
              <div>
                <h4 className="text-sm font-bold text-gray-900 group-hover:text-blue-600 transition">{settings?.authorAnnounce1Title ?? "Selamat Datang!"}</h4>
                <p className="text-xs text-gray-500 mt-1 leading-relaxed">{settings?.authorAnnounce1Desc ?? "Sistem v2.0 telah aktif."}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-[2rem] border border-gray-700 p-6 md:p-8 shadow-2xl relative overflow-hidden flex flex-col justify-between group">
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 bg-amber-500/20 text-amber-400 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest mb-4 border border-amber-500/30"><Crown size={12} className="fill-amber-400" /> Fitur Eksklusif</div>
            <h2 className="text-2xl font-black text-white mb-2 leading-tight">{renderPromoPremiumTitle(settings?.promoPremiumTitle)}</h2>
            <p className="text-gray-400 text-sm leading-relaxed mb-6">{settings?.promoPremiumDesc ?? "Dapatkan ribuan pembaca baru."}</p>
          </div>
      ...
      <section className="bg-white p-6 md:p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 border-b border-gray-100 pb-4">
          ...
          <Link href="/author/new" className="bg-indigo-600 text-white px-6 py-3 rounded-xl text-sm font-black flex items-center justify-center gap-2 hover:bg-indigo-700 transition shadow-lg active:scale-95">
            ...
            {userNovels.map((novel) => (
              <Link key={novel.id} href={`/author/novel/${novel.slug}`} className="bg-white rounded-[2rem] border border-gray-100 p-3 shadow-sm hover:border-indigo-300 hover:shadow-xl transition-all duration-300 block group transform hover:-translate-y-1">
                <div className="aspect-[2/3] bg-gray-100 rounded-2xl mb-4 overflow-hidden relative shadow-inner">
                  {novel.coverImage ? (
                    <img src={novel.coverImage} alt={novel.title} className="w-full h-full object-cover group-hover:scale-110 transition duration-700" loading="lazy" />
                  ) : (
                    <div className="flex items-center justify-center w-full h-full text-gray-300 bg-gray-50"><BookOpen size={40} /></div>
                  )}
                </div>

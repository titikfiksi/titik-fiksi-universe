...
import { Star, Eye, BookOpen, ExternalLink, PenTool, Instagram, Twitter, Facebook, Youtube, Link as LinkIcon, MessageCircle, Wrench, Heart } from "lucide-react";
...
import PagedChapterList from "@/components/PagedChapterList"; // <-- Import Komponen Baru

export const revalidate = 60; 

// ====================================================================
// GENERATOR WAJAH SOSIAL MEDIA (OPEN GRAPH & SEO)
// ====================================================================
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  if (!novel) {
    return { title: 'Novel Tidak Ditemukan | Titik Fiksi Universe' };
  }

  const safeDescription = novel.synopsis 
    ? novel.synopsis.substring(0, 160) + '...' 
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: novel.coverImage || 'https://i.ibb.co.com/7xj5JTgp/Poster-2.jpg', 
          width: 800,
          height: 600,
...
function getSocialIcon(platform: string, size = 14) {
  const p = platform.toLowerCase();
  if (p.includes("instagram") || p.includes("ig")) return <Instagram size={size} />;
...
export default async function NovelDetailPage({ params }: { params: { slug: string } }) {
  ...
  const avgRating = totalRatings > 0 ? (novel.ratings.reduce((a, b) => a + b.value, 0) / totalRatings).toFixed(1) : "0.0";
  
  // SEMUA TIPE DATA ANY SUDAH DIHAPUS KARENA PRISMA SUDAH MENGENALINYA
  const authorSocialsData = novel.authorSocials || [];
  const authorDonation = novel.authorDonationUrl; 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12">
          ...
          <div className="lg:col-span-4 space-y-6 md:space-y-8">
            <div className="relative aspect-[2/3] rounded-3xl overflow-hidden shadow-2xl border border-gray-200 bg-white group w-2/3 mx-auto md:w-full">
              ...
              <div className="absolute top-4 left-4 bg-black/70 backdrop-blur-md text-yellow-400 font-black px-4 py-2 rounded-2xl flex items-center gap-2 shadow-lg z-10"><Star size={18} fill="currentColor"/> {avgRating}</div>
            </div>
            ...
            <div className="bg-white p-5 md:p-6 rounded-3xl border border-gray-100 shadow-sm space-y-4">
              ...
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 text-xs md:text-sm font-bold border-b border-gray-50 pb-4">
                <span className="text-gray-400 uppercase tracking-widest pt-1">Penulis</span>
                <div className="flex flex-col sm:items-end gap-2">
                  <span className="text-gray-900 flex items-center gap-2 bg-emerald-50 px-3 py-1.5 rounded-xl border border-emerald-100">
                    <PenTool size={14} className="text-emerald-600"/> {novel.author || "Lutfi Abdulloh"}
                  </span>
                  
                  <div className="flex flex-wrap justify-start sm:justify-end gap-1.5 mt-1">
                    {authorDonation && (
                      <a href={authorDonation} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 bg-gradient-to-r from-pink-500 to-rose-500 text-white text-[10px] font-bold px-2.5 py-1.5 rounded-lg shadow-md hover:shadow-lg transition-transform hover:-translate-y-0.5">
                        <Heart size={12} className="fill-current animate-pulse"/> Dukung Penulis
                      </a>
                    )}
                    
                    {authorSocialsData.map((social) => (
                      <a key={social.id} href={social.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 bg-gray-900 text-white text-[10px] font-bold px-2.5 py-1.5 rounded-lg hover:bg-emerald-600 hover:shadow-md transition-all transform hover:-translate-y-0.5" title={social.platform}>
                  ...
                  {novel.genre.split(',').map((g, index) => {
                    ...
                    return (
                      <Link 
                        ...
                        title={`Cari novel dengan genre ${cleanGenre}`}
                      >
                        {cleanGenre}
                      </Link>
                    )
                ...
                <div className="grid grid-cols-1 gap-2">
                  {novel.externalLinks.map((link) => (
                    <a key={link.id} href={link.url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-4 bg-white border border-gray-200 rounded-2xl hover:border-blue-500 transition shadow-sm group">
                      <span className="font-bold text-gray-700 text-sm group-hover:text-blue-600">{link.title}</span><ExternalLink size={16} className="text-gray-300 group-hover:text-blue-500 flex-shrink-0" />
                    </a>
                  ))}
                </div>
              </div>
            ...
            <div className="flex flex-col gap-3">
              <ContinueReadingButton novelSlug={novel.slug} chapters={novel.chapters} />
          ...
          <div className="lg:col-span-8 space-y-10 md:space-y-12">
            ...
            <div className="space-y-6">
              ...
              {/* IMPLEMENTASI TAHAP 5.3: MEMANGGIL KOMPONEN DAFTAR ISI CERDAS */}
              <PagedChapterList chapters={novel.chapters} novelSlug={novel.slug} />

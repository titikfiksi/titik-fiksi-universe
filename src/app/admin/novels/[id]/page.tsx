...
import {
  ArrowLeft, Plus, Edit, BookOpen, Link as LinkIcon, Save, Image as ImageIcon, Layout,
  Lock, PenTool, AtSign, Heart, MessageSquare, Reply, Crown, Star
} from "lucide-react";
import {
  deleteChapter, addExternalLink, deleteExternalLink, updateNovel, addAuthorSocial,
  deleteAuthorSocial, deleteComment, replyCommentByAdmin, toggleFeaturedNovel
} from "@/lib/actions";
import DeleteButton from "@/components/DeleteButton";
import type { Novel, Chapter, Comment, ExternalLink, AuthorSocial } from "@prisma/client";
...
const PREDEFINED_GENRES = [
  ...
  "Mystery", "Romance", "Sci-Fi", "System", "Urban Fantasy"
] as const;

type ExtendedChapter = Chapter & {
  comments: Comment[]
};
type ExtendedNovel = Novel & {
  chapters: ExtendedChapter[];
  externalLinks: ExternalLink[];
  authorSocials: AuthorSocial[];
};

function parseGenres(genres: string) {
  const savedGenres = genres.split(',').map((g) => g.trim()).filter(Boolean);
  const customGenres = savedGenres.filter((g) => !PREDEFINED_GENRES.includes(g as any));
  return { savedGenres, customGenresString: customGenres.join(", ") };
}

function getRecentComments(novel: ExtendedNovel) {
  const comments = novel.chapters.flatMap((ch) =>
    ch.comments.map((c) => ({
      ...c,
  ...
  // Use toISOString for safer serialization (if needed) in strict TS environments
  return comments
    .sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())
    .slice(0, 15);
}

// Banner featured toggle handler (needs to be inside server component)
async function handleToggleFeatured(novelId: string, formData: FormData) {
  "use server";
  const daysVal = formData.get("days");
  const days = daysVal ? parseInt(daysVal as string, 10) : null;
  await toggleFeaturedNovel(novelId, days);
}
    ...
    <div className="max-w-6xl mx-auto space-y-10 pb-20 pt-28 px-4 sm:px-6 lg:px-8 animate-fade-in-up">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <main className="lg:col-span-8 space-y-8">
          ...
          >
            <div className="p-6 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
              <h2 className="font-bold text-gray-800 flex items-center gap-2">
                <Edit size={18} /> Informasi Novel & Penulis
              </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-black text-gray-400 uppercase tracking-widest flex items-center gap-1">
                    <PenTool size={12}/> Penulis Utama
                  </label>
                  <input
                    type="text"
                    name="author"
                    defaultValue={String((novel as any).author ?? "Lutfi Abdulloh")}
                    required
                ...
                <div className="space-y-2">
                  <label className="text-xs font-black text-gray-400 uppercase tracking-widest flex items-center gap-1">
                    <Heart size={12}/> Link Donasi Penulis
                  </label>
                  <input
                    type="url"
                    name="authorDonationUrl"
                    defaultValue={String((novel as any).authorDonationUrl ?? "")}
              ...
              novel.chapters.length === 0 ? (
                ...
                <ul className="space-y-3">
                  {novel.chapters.map((chapter) => (
                    <li key={chapter.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 group">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <Link
                          href={`/admin/novels/${novel.id}/chapters/${chapter.id}`}
                        ...
                        <form action={deleteChapter.bind(null, chapter.id, novel.id)}>
                          <DeleteButton message={`Hapus Bab ${chapter.orderIndex}?`} />
                        </form>
                      </div>
                    </li>
                  ))}
          ...
          {/* ================== Komentar Section ================== */}
          <section className="bg-white p-6 md:p-8 rounded-[2rem] border border-gray-200 shadow-sm">
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
              <h2 className="text-xl font-black text-gray-800 flex items-center gap-2">
                <MessageSquare className="text-pink-500" size={24}/> Komentar Pembaca
              </h2>
            </div>
            {
                  ...
                  {allComments.map(comment => (
                    <div key={comment.id} className="p-4 bg-gray-50 rounded-2xl border border-gray-100 relative group">
                      ...
                      <details className="mt-4 group/reply">
                        ...
                        <form
                          action={replyCommentByAdmin.bind(null, comment.chapterId, novel.id)}
        ...
        {/* ================== Sidebar (Banner + Cover + External Links + Sosmed) ================== */}
        <aside className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-gradient-to-br from-gray-900 to-black p-6 rounded-[2rem] border border-gray-800 shadow-xl text-white relative overflow-hidden">
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between gap-4 mb-4">
                <div>
                  <h3 className="text-amber-400 font-black flex items-center gap-2">
                    <Crown size={18} className="fill-amber-400"/> Sorotan Utama
                  </h3>
              ...
              <form
                action={handleToggleFeatured.bind(null, novel.id)}
          ...
          >
            ...
            />
            {/* Ensure title is passed (needed by many updateNovel functions) */}
            <input type="hidden" name="title" value={novel.title} />
            <button
              type="submit"
              className="w-full flex items-center justify-center gap-2 bg-purple-600 text-white py-3 rounded-xl text-sm font-bold hover:bg-purple-700 transition shadow-sm"
            >
          ...
          <section className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm">
            <h2 className="font-bold text-gray-800 mb-4 flex items-center gap-2 pb-4 border-b border-gray-100">
              <LinkIcon className="text-indigo-600" size={18}/> Link External
            </h2>
            <form
          ...
          <section className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm">
            <h2 className="font-bold text-gray-800 mb-4 flex items-center gap-2 pb-4 border-b border-gray-100">
              <AtSign className="text-emerald-600" size={18}/> Sosial Media Penulis
            </h2>
            <form
            ...
            <div className="space-y-2">
              {novel.authorSocials.map(social => (
                <div key={social.id} className="flex items-center justify-between p-3 bg-emerald-50 rounded-xl border border-emerald-100">
                  ...
                  <form action={deleteAuthorSocial.bind(null, social.id, novel.id, novel.slug)}>
                    <DeleteButton message="Hapus?" />
                  </form>
                </div>
              ))}

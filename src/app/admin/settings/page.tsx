        ...
        <div className="md:col-span-3 space-y-8">
              ...
              <div className="p-8 space-y-6">
                ...
                <div className="flex items-center justify-between p-5 bg-blue-50 rounded-2xl border border-blue-200">
                  <div>
                    ...
                    <span className="text-xs text-blue-700">Matikan jika sedang perbaikan.</span>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="isActive" defaultChecked={settings?.isActive} className="sr-only peer" />
                    <div className="w-11 h-6 bg-blue-200 rounded-full peer peer-checked:bg-blue-600 after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-full"></div>
                  </label>
                </div>
                <div>
                  <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Teks Hak Cipta (Footer)</label>
                  <input type="text" name="copyrightText" defaultValue={settings?.copyrightText || ""} className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl outline-none" />
                </div>
              </div>
            </form>
          )}
          ...
          {activeTab === "akun" && (
            <form action={updateSettings} className="bg-white rounded-[2.5rem] border border-red-200 shadow-sm overflow-hidden animate-fade-in-up">
              ...
              <div className="p-8 space-y-6">
                <div className="bg-red-50 border border-red-100 p-5 rounded-2xl flex items-start gap-3 text-sm font-medium text-red-800">
                  <ShieldAlert className="text-red-500 flex-shrink-0" size={24}/>
                  <p>Wajib masukkan Sandi Saat Ini jika Anda mengubah Email atau Sandi Baru.</p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <input type="email" name="adminEmail" defaultValue={settings?.adminEmail || settings?.email || ""} required className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl font-bold" />
                  <PasswordInput name="newAdminPassword" placeholder="Sandi Baru (Opsional)" className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl" />
                </div>
                <div className="pt-6 border-t border-gray-100 w-full md:w-1/2">
                  <label className="block text-[10px] font-black text-red-600 uppercase mb-2"><Lock size={12} className="inline mr-1"/> Konfirmasi Sandi Saat Ini</label>
                  <PasswordInput name="oldAdminPassword" placeholder="Wajib diisi" className="w-full px-4 py-3 bg-red-50 border border-red-200 rounded-xl" />
                </div>
              </div>
            </form>
          )}
             ...
             <div className="space-y-8 animate-fade-in-up">
              <div className="bg-white p-8 rounded-[2.5rem] border border-red-200 shadow-sm">
                ...
                <form action={addDonationLink} className="space-y-3 mb-6">
                  <input type="text" name="platform" placeholder="Platform (Saweria/Trakteer)" required className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold outline-none" />
                  <input type="url" name="url" placeholder="URL Link Donasi" required className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none" />
                  <SubmitButton text="Tambah Donasi" icon={<Plus size={16}/>} customClass="w-full bg-red-600 text-white text-sm font-bold rounded-xl py-3 flex items-center justify-center gap-2" />
                </form>
              ...
              <div className="bg-white p-8 rounded-[2.5rem] border border-indigo-200 shadow-sm">
                ...
                <form action={addSponsor} className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                   <input type="text" name="title" placeholder="Nama Produk/Sponsor" required className="p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm md:col-span-2" />
                   <MultiImageInput />
                   <input type="url" name="linkUrl" placeholder="Link Tujuan Pembelian" required className="p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm md:col-span-2" />
                   <textarea name="description" placeholder="Deskripsi Singkat" className="p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm md:col-span-2"></textarea>
                   <SubmitButton text="Tambah Produk" icon={<Plus size={16}/>} customClass="md:col-span-2 flex items-center justify-center gap-2 py-3 bg-indigo-600 text-white rounded-xl font-bold shadow-md" />
                </form>
                  ...
                  {sponsors.map(s => (
                    <div key={s.id} className="flex flex-col p-4 bg-indigo-50 rounded-2xl border border-indigo-100 gap-3 relative overflow-hidden">
                      ...
                      <div className="flex gap-2 overflow-x-auto pb-1">
                        {s.imageUrl.split(",").map((imgUrl: string, i: number) => (
                          <img key={i} src={imgUrl} alt="Produk" className="w-12 h-12 object-cover rounded-lg border border-indigo-200" />
                        ))}
                      </div>
                    </div>
                  ))}
          ...
          {activeTab === "tentang" && (
            <form action={updateSettings} className="bg-white rounded-[2.5rem] border border-amber-200 shadow-sm overflow-hidden animate-fade-in-up">
              ...
              <div className="p-6 border-b border-amber-100 bg-amber-50/50 flex items-center justify-between">
                ...
                <SubmitButton text="Simpan" icon={<Save size={16}/>} customClass="bg-amber-600 text-white px-6 py-2 rounded-xl text-sm font-bold flex items-center gap-2 shadow-md" />
              </div>
              <div className="p-8 space-y-6">
                <textarea name="visiPenulis" defaultValue={settings?.visiPenulis || ""} rows={4} placeholder="Visi Penulis" className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl outline-none font-medium leading-relaxed"></textarea>
                <textarea name="kekuatanPembaca" defaultValue={settings?.kekuatanPembaca || ""} rows={4} placeholder="Kekuatan Pembaca" className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl outline-none font-medium leading-relaxed"></textarea>
              </div>
            </form>
          )}

          {/* TAB: KONTAK (DENGAN SOSIAL MEDIA) */}
          {activeTab === "kontak" && (
            <div className="space-y-8 animate-fade-in-up">
              <form action={updateSettings} className="bg-white rounded-[2.5rem] border border-sky-200 shadow-sm overflow-hidden">
                ...
                <div className="p-6 border-b border-sky-100 bg-sky-50/50 flex items-center justify-between">
                  ...
                  <SubmitButton text="Simpan" icon={<Save size={16}/>} customClass="bg-sky-600 text-white px-6 py-2 rounded-xl text-sm font-bold flex items-center gap-2 shadow-md" />
                </div>
                <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-xs font-black text-sky-800 uppercase tracking-widest mb-2">Email Publik</label>
                    <input type="email" name="email" defaultValue={settings?.email || ""} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none font-bold" />
                  </div>
                  <div>
                    <label className="block text-xs font-black text-sky-800 uppercase tracking-widest mb-2">No. WhatsApp Utama</label>
                    <input type="text" name="whatsappNumber" defaultValue={settings?.whatsappNumber || ""} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none font-bold" />
                  </div>
                </div>
              </form>
              <div className="bg-white p-8 rounded-[2.5rem] border border-gray-200 shadow-sm">
                ...
                <form action={addSocialLink} className="space-y-3 mb-6">
                  <input type="text" name="platform" placeholder="Platform (Instagram/TikTok)" required className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold" />
                  <input type="url" name="url" placeholder="URL Profil" required className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm" />
                  <SubmitButton text="Tambah Sosmed" icon={<Plus size={16}/>} customClass="w-full bg-blue-600 text-white text-sm font-bold rounded-xl py-3 flex items-center justify-center gap-2" />
                </form>
                <div className="space-y-2">
                  {socials.map((s: any) => (
          ...
          {activeTab === "penulis" && (
            <form action={updateSettings} className="bg-white rounded-[2.5rem] border border-emerald-200 shadow-sm overflow-hidden animate-fade-in-up">
              ...
              <div className="p-6 border-b border-emerald-100 bg-emerald-50/50 flex items-center justify-between">
                ...
                <SubmitButton text="Simpan" icon={<Save size={16}/>} customClass="bg-emerald-600 text-white px-6 py-2 rounded-xl text-sm font-bold flex items-center gap-2 shadow-md" />
              </div>
              <div className="p-8 space-y-8">
                <div className="flex items-center justify-between p-5 bg-emerald-50 rounded-2xl border border-emerald-200">
                  <span className="font-bold text-emerald-900 text-sm">Buka Pendaftaran (Menu Gabung Penulis)</span>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="isOpenForWriters" defaultChecked={settings?.isOpenForWriters ?? true} className="sr-only peer" />
                    <div className="w-11 h-6 bg-emerald-200 rounded-full peer peer-checked:bg-emerald-600 after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-full"></div>
                  </label>
                </div>
                <div className="space-y-4">
                  <input type="text" name="writerHeroTitle" defaultValue={settings?.writerHeroTitle || ""} placeholder="Judul Panggilan" className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl font-bold" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[1,2,3,4].map(i => (
                    <div key={i} className="bg-emerald-50 p-4 rounded-xl border border-emerald-100 space-y-3">
                      <label className="block text-[10px] font-black text-emerald-800 uppercase">Kotak Keuntungan {i}</label>
                      <input type="text" name={`writerBenefit${i}Title`} defaultValue={settings?.[`writerBenefit${i}Title`] || ""} placeholder="Judul..." className="w-full p-2.5 rounded-lg text-sm font-bold" />
                      <textarea name={`writerBenefit${i}Desc`} defaultValue={settings?.[`writerBenefit${i}Desc`] || ""} rows={2} placeholder="Deskripsi..." className="w-full p-2.5 rounded-lg text-xs"></textarea>
                    </div>
                  ))}
                </div>
                <textarea name="writerTerms" defaultValue={settings?.writerTerms || ""} rows={6} placeholder="Syarat & Ketentuan Naskah" className="w-full p-4 bg-white border border-emerald-200 rounded-xl outline-none font-medium"></textarea>
              </div>
            </form>
          )}
          ...
          {activeTab === "dasbor_kreator" && (
            <form action={updateSettings} className="bg-white rounded-[2.5rem] border border-purple-200 shadow-sm overflow-hidden animate-fade-in-up">
              ...
              <div className="p-6 border-b border-purple-100 bg-purple-50/50 flex items-center justify-between">
                ...
                <SubmitButton text="Simpan" icon={<Save size={16}/>} customClass="bg-purple-600 text-white px-6 py-2 rounded-xl text-sm font-bold flex items-center gap-2 shadow-md" />
              </div>
              <div className="p-8 space-y-8">
                <div className="space-y-4">
                   <h3 className="text-sm font-black text-gray-400 uppercase tracking-widest border-b border-gray-100 pb-2">Pengumuman 1</h3>
                   <input type="text" name="authorAnnounce1Title" defaultValue={settings?.authorAnnounce1Title || ""} placeholder="Judul Pengumuman 1" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl font-bold" />
                   <textarea name="authorAnnounce1Desc" rows={2} defaultValue={settings?.authorAnnounce1Desc || ""} placeholder="Isi Pengumuman 1" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl"></textarea>
                </div>
                <div className="space-y-4 p-5 bg-gray-900 rounded-2xl border border-gray-700">
                   ...
                   <input type="text" name="promoPremiumTitle" defaultValue={settings?.promoPremiumTitle || ""} placeholder="Judul Banner" className="w-full p-3 bg-gray-800 border border-gray-600 text-white rounded-xl font-black outline-none focus:ring-2 focus:ring-amber-500" />
                   <textarea name="promoPremiumDesc" rows={2} defaultValue={settings?.promoPremiumDesc || ""} placeholder="Isi Promo" className="w-full p-3 bg-gray-800 border border-gray-600 text-gray-300 rounded-xl outline-none"></textarea>
                </div>
              </div>
            </form>
          )}

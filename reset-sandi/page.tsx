...
import ResetSandiClient from "@/components/ResetSandiClient";
...
function InvalidToken() {
  return (
    <div className="relative z-10 w-full max-w-[420px] bg-white p-8 md:p-10 rounded-[2rem] shadow-2xl border border-gray-100 animate-fade-in-up text-center">
      ...
      <p className="text-gray-500 font-medium text-sm mb-8 leading-relaxed">Tautan pemulihan tidak valid atau hilang. Silakan buat permintaan lupa kata sandi yang baru.</p>
...
export default async function ResetSandiPage({ searchParams }: { searchParams: Record<string, string | string[] | undefined> }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center items-center p-4 pt-28 relative overflow-hidden">
      ...
      {!token ? <InvalidToken /> : <ResetSandiClient token={token} />}
    </div>
  );
}
